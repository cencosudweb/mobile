...
<html>
  <head>
    ...
    <script src="<c:url value="/js/admin/tienda/tienda.all.js"/>" type="text/javascript"></script>
        ...
        <div class="container-fluid">
	        <div class="row-fluid">
				<div class="btn-toolbar">
				    <button id="btn-add-tienda" class="btn btn-primary"><i class="icon-plus"></i> Nuevo</button>
				    <!--  
				...
				<div class="well">
					  ...
					  <c:if test="${!empty tiendas}">
					      <c:forEach items="${tiendas}" var="tienda" varStatus="status">
					            <tr>
						          ...
						          <td nowrap="nowrap">${tienda.description}</td>
						          <td>
						          	  <!--  <a href="resetPassword1?id=${user.id}" title="Restablecer Clave" ><i class="icon-signin"></i> Restablecer Pass</a><br />-->
						              <a href="editTienda?id=${tienda.id}" title="Editar" ><i class="icon-pencil"></i>Editar</a><br />
						              <a href="#myModal" class="lockUnlock" modo='U' idtienda="${tienda.id}" title="Deshabilitar" role="button" data-toggle="modal" style="margin-right: 15px"><i class="icon-remove"></i>Eliminar</a><br />
						          </td>
						        </tr>
						</c:forEach>
				      </c:if>
					...
					<form method="post"  action="lockUnlockTienda" id="f_lock">
		        			<input type="hidden" name="id" id="id_tienda"/>
