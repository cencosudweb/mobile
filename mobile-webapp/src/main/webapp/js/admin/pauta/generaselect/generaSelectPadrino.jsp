...
try{
	...
	SQL = "Select id,description from padrinos WHERE id_tienda = "+id;
	...
	conn.commit();
	out.println("<option value=\"\">Seleccionar Padrino</option>");
	...
	while(rs.next()) {
		...
		%>
		<% //if (request.getParameter("region") == rs.getString("id")) { %> <!--  selected--><% //} %>
		
		<%
		//out.println(">"+ rs.getString("description")+"</option>");
		%>
